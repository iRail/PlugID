...
class Service_wordpress extends Service_driver {

    private $oauth, $access_token;
    private $url_authorize = 'https://public-api.wordpress.com/oauth2/authorize';
    private $url_access_token = 'https://public-api.wordpress.com/oauth2/token';
    private $url_base = 'https://public-api.wordpress.com/rest/v1/';
    ...
    function callback($data) {
        $error_message = 'Error authenticating with Wordpress. Please try again later. Technical detail for our monkeys: ';
        // check code
        if (!isset($data['code'])) {
        ...
        if (is_null($user) || !isset($user->ID)) {
            show_error($error_message . "Error while retrieving UserID from Wordpress");
        }
